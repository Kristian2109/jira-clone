...
class ProjectManager {
  ...
  public async inviteUser(projectId: number, userEmail: string) {
    const project =
      await this._projectCustomRepository.findByIdWithMembersAndViews(
        projectId
      );
    const user = await this._userManager.getUserByEmail(userEmail);

    const isUserAlreadyMember = project.members.find(
  ...
  public async acceptInvitation(projectId: number, userId: number) {
    const invitation = await this._projectCustomRepository.findMembership(
      projectId,
      userId
    );
